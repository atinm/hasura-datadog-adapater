    ...
    match health_check {
        Ok(v) => {
            if v.status() == reqwest::StatusCode::OK {
                ...
                dd.service_check("health", datadog_statsd::client::ServiceCheckStatus::Ok, &None);
            } else {
                debug!("Healthcheck NOK");
                dd.service_check("health", datadog_statsd::client::ServiceCheckStatus::Critical, &None);
            }
        },
        Err(e) => {
            dd.service_check("health", datadog_statsd::client::ServiceCheckStatus::Unknown, &None);
